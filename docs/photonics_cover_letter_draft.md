...
The manuscript is original, has not been published previously, and is not under consideration elsewhere. [Add any funding, conflict-of-interest, ethics, or data-availability statements here if the journal requires them in the cover letter.] A public development repository is provided, with the submission-aligned public code snapshot fixed at commit `708d8bb3610f`; the repository is enabled for Zenodo archiving under `dawidkucharski/Quantum_interference`, allowing a DOI-bearing release to be minted from the final GitHub submission release. The manuscript also identifies a reduced experimental-validation subset and protocol for the next laboratory stage.
