% Screening flow for planning follow-up validation derived from the grouped benchmark trends
\begin{tikzpicture}[x=1cm,y=1cm, font=\footnotesize, >=Stealth]
  ...
  \node[head] (start) at (0,0) {\textbf{Follow-up validation screening}\\Prioritise the branch that matches the endpoint worth testing first.};
  ...
  \node[mbox] (classic) at (-4.4,-5.0) {\textbf{Classical PSI}\\Baseline to validate first\\Best fit for rougher classes: grinding, rough turning, milling finishing, wire EDM.};
  \node[mbox] (hybrid) at (0,-5.0) {\textbf{Hybrid}\\First nonclassical follow-up\\Use when $S_a$ matters or when coincidence information should only stabilise unwrapping.};
  \node[mbox] (quantum) at (4.4,-5.0) {\textbf{Coincidence-proxy}\\Selective follow-up only\\Most defensible on smoother finishing-style regimes and mainly for $S_z$.};

  \node[note] (fallback) at (0,-7.55) {If the regime is unclear, validate \textbf{classical} first and treat \textbf{hybrid} as the next follow-up before direct coincidence inversion.};
