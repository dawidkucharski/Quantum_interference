...
\dataavailability{The Git-tracked project snapshot associated with this study contains the manuscript sources, simulation code, and the generated benchmark artefacts cited in the paper. Canonical regeneration is driven by \path{scripts/run_paper_measured_benchmark.py}, \path{scripts/run_paper_optprops.py}, and \path{scripts/select_experimental_validation_subset.py}; targeted controls, canonical sources, output folders, and revision-support files are enumerated in \path{docs/submission_snapshot_manifest.md}. The reduced experimental-validation subset is included under \path{outputs/paper_alicona_benchmark/experimental_validation/} together with the accompanying protocol in \path{docs/experimental_validation_protocol.md}. A public development repository is available at \url{https://github.com/dawidkucharski/Quantum_interference}, and the submission-aligned public snapshot for the present manuscript is fixed by commit \texttt{708d8bb3610f}. The repository is enabled for Zenodo archiving under \texttt{dawidkucharski/Quantum\_interference}, allowing a DOI-bearing Zenodo release to be minted from the corresponding GitHub release for the final manuscript snapshot. The full local set of measured input \texttt{.sur} topographies is excluded from the normal Git history because the raw dataset is too large for a practical public GitHub snapshot; those raw SUR files remain available from the corresponding author upon reasonable request. The available public snapshot supports regeneration of the simulation benchmark, figures, tables, and reduced-validation planning record. Any future experimental validation dataset should be archived separately with the corresponding independent reference measurements.}
