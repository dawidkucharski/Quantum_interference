...
\abstract{This paper presents a simulation benchmark of interferometric reconstruction architectures for rough engineering surfaces by comparing classical four-step phase-shifting interferometry (PSI), coincidence-proxy channels, and hybrid coarse-to-fine reconstruction under matched detected-count assumptions. Fifty-nine focus-variation (FV) topographies exported as Mountains/DigitalSurf \texttt{.sur} files provide measured geometric priors for simulated optical observations, so the same material- and process-dependent surfaces can be tested across classical, direct coincidence-proxy, and hybrid pipelines as architecture-screening inputs. The clearest result is at architecture-level height fidelity: on the measured-surface benchmark, the hybrid branch gives the lowest surface-level median detrended height RMSE (314.0~nm), wins on 32 of 59 surfaces, remains below an optimistic classical-frontier oracle (559.1~nm), and retains the same ordering in a rate-based coincidence control (290.9~nm under ideal matched-count rates; 376.3~nm under non-ideal detector assumptions, compared with 1308.3~nm for the direct branch). Roughness endpoints define the boundary of this claim rather than overturning it. On the matched-bandwidth benchmark-grid reference, hybrid gives the lowest median absolute Sa and Sq errors (86.3 and 152.4~nm), while the direct coincidence-proxy branch is selectively best for absolute Sz (2653.1~nm); treatment holdouts remain process dependent (70.0\%, 60.0\%, and 60.0\% agreement for Sa, Sq, and Sz), and approximate Gaussian S/L filtering does not make the roughness ranking portable. Classical two-colour and classical-frontier controls show that direct long-wavelength envelope behaviour can also be reproduced classically, but they do not remove the hybrid height-fidelity advantage. The contribution is therefore a stress-tested photonic-architecture screen: coincidence-derived information is most valuable as an ambiguity-resolving or absolute-height prior inside a coarse-to-fine estimator, while final fine texture remains anchored by the short-wavelength PSI branch. Because the FV surfaces are used as common geometric priors and not as traceable reference standards, the result is a reproducible architecture-screening benchmark and hardware-follow-up priority map.}
...
This structure deliberately turns claim selection into a controlled stress test. A direct coincidence-proxy channel is advanced only where it improves the target endpoint itself; a hybrid architecture is justified where the proxy observable carries useful coarse or absolute-height information while the short-wavelength PSI branch preserves fine texture. Because the FV topographies provide a shared geometric prior for simulated optical observations, the measured-surface branch is an architecture-screening benchmark rather than a traceable validation study. Within that scope, the paper reports height RMSE after detrending, roughness-parameter biases, bootstrap uncertainty bounds, tolerance-exceedance summaries, holdout-stability checks, per-surface dominance counts, Holm-adjusted paired tests, paired-effect summaries, spectral-fidelity comparisons, and a structured failure taxonomy, so that the benchmark output becomes a reduced-subset experimental prioritisation map rather than a deployment rule.

\paragraph{Scope of claims.}
To make the claim boundary explicit, the manuscript does not assert a demonstrated quantum advantage, a calibrated coincidence-metrology instrument, traceable roughness certification, or a deployment-ready method-selection rule. The intended claim is narrower and testable: within a fixed, reproducible simulation hierarchy seeded by measured FV topographies, coincidence-derived information is most defensible as a coarse ambiguity-resolving prior in a hybrid estimator, while the short-wavelength PSI branch should carry the final fine texture. The manuscript is therefore positioned as a photonic-instrumentation architecture screen and experimental-priority map for hardware follow-up.
...
Manuscript-ready tables are written into \path{manuscript/tables}, the paper is compiled from \path{manuscript/main.tex} with \path{latexmk -pdf -bibtex -interaction=nonstopmode main.tex}, and the exact submission snapshot is indexed in \path{docs/submission_snapshot_manifest.md}. A Q1 reproducibility generator exports the release companion package with derived $256\times256$ benchmark-grid surfaces, public CSV summaries, generated table sources, exact regeneration commands, and a SHA-256 checksum so that reviewers can audit the simulation benchmark without requiring the full native \texttt{.sur} files. The repository also contains smoke runs, exploratory sweeps, and intermediate plots used during development; those files are excluded from the submission record.
...
To make the next stage executable rather than generic, the submission snapshot fixes a reduced experimental-validation subset and corresponding protocol. Table~\ref{tab:experimental_validation_subset} summarises the six core and two extended surfaces chosen to test the best-case hybrid claim, a representative median case, the least favourable direct-branch case, the grouped rough-turning and rough-WEDM exceptions, the classical two-colour comparator case, the detector-sensitivity case, and the strongest native-grid $S_z$ envelope-following case. The accompanying protocol requires repeated acquisition of the classical, classical two-colour, direct coincidence, and hybrid branches on the same field of view, together with matched-bandwidth reporting, an uncertainty log, and a mandatory independent reference cross-check on every core case.

\input{tables/experimental_validation_subset.tex}

\paragraph{Limitations of the benchmark.}
The present work is a simulation benchmark built on real measured topographies rather than an experimental coincidence-metrology demonstration. The coincidence branch is not yet calibrated against a laboratory fourth-order interference transfer function, and the strongest rate-based control is therefore a sensitivity study rather than a closure of the underlying quantum optics. The FV surfaces are operational benchmark inputs, not traceable reference standards: no independent instrument cross-check, ISO-style $S/L$ filtering, or GUM-style uncertainty budget is applied in the present results, so the reported roughness and PSD outcomes inherit FV bandwidth, outlier sensitivity, and the deterministic low-pass effect of the benchmark-grid area averaging. Because both the simulator input surface and the scoring reference are derived from the same FV export, the measured-surface branch is not an external validation study; it is an internal-consistency benchmark under a shared geometric prior. The primary manuscript roughness comparison is therefore restricted to the bandwidth-consistent benchmark grid, while the retained native-grid descriptor view is only a harsher diagnostic that demonstrates sensitivity to reference bandwidth and outlier structure rather than a second co-equal ranking.

The generalisation limits are likewise narrow. The bootstrap intervals and holdouts are descriptive summaries of the fixed simulator/reference dataset, while the Holm-adjusted paired Wilcoxon screen is a dataset-level statistical comparison rather than a metrological uncertainty budget or inter-instrument reproducibility study. The material/treatment holdouts are adequate for an architecture-level height-RMSE screen and remain weaker for roughness once the 59 surfaces are split across six materials and ten treatments: matched-bandwidth treatment-holdout agreement is 70\% for $|\Delta S_a|$, 60\% for $|\Delta S_q|$, and 60\% for $|\Delta S_z|$, so the roughness descriptors should be interpreted as process-dependent diagnostics rather than portable screening rules. The matched-count comparison is also only one fairness convention; detector efficiency, accidentals, and deadtime materially move the direct-branch numbers, which is why the non-ideal rate control should be read as the more conservative deployment-facing bound. Finally, the benchmark spans six materials and ten treatments but not polished optics, additively manufactured surfaces, or other fundamentally different texture families. The present evidence therefore supports a screening and prioritisation conclusion: coincidence-derived information remains worth testing as a coarse ambiguity-resolving prior, and the reduced subset plus protocol in Table~\ref{tab:experimental_validation_subset} identify the most efficient path for the next experimental stage rather than leaving follow-up work undefined.

The added approximate Gaussian $S/L$ roughness-filter control is therefore best interpreted as a boundary check on roughness portability. Because the filtered treatment holdouts remain at 60\% for all three descriptors and filtered $|\Delta S_q|$ material portability weakens rather than strengthens, an ISO-style filtering surrogate alone does not turn the roughness ranking into a portable workflow rule. The control is nevertheless useful because it shows that the roughness endpoint split is not merely an artefact of the raw detrend-only descriptor path.
...
Within that scope, the paper contributes a positive architecture-selection result. It identifies the architecture worth carrying forward, quantifies the detector, drift, ambiguity, and bandwidth conditions that shape its performance, and separates the strong height-fidelity result from the weaker, process-dependent roughness picture. The hybrid branch remains the best measured-surface architecture at 314.0~nm median RMSE and 32 of 59 per-surface wins, and the stronger rate-based coincidence model preserves the same ordering: under ideal matched-count rates the hybrid branch remains best at 290.9~nm, while under non-ideal detector assumptions it rises only to 376.3~nm as the direct branch degrades to 1308.3~nm. Roughness endpoints do not support a universal workflow rule, but they remain informative because they show where the architectural claim stops being portable: matched-bandwidth $S_a$ and $S_q$ favour hybrid within the primary branches, while the remaining matched-bandwidth $S_z$ advantage of the coincidence-proxy branch remains treatment dependent and still exceeds descriptor-scaled tolerances on 42.4\% of surfaces. The classical-frontier oracle and the classical two-colour control therefore narrow the claim without erasing it: the main result is a well-controlled architectural benchmark, not a broad stand-alone quantum-advantage claim, and it identifies hybrid coincidence-assisted profilometry as the most credible configuration for experimental follow-up when benchmark-grid height fidelity is the primary endpoint.

For future work, the next step is now concrete rather than generic. The current submission snapshot already fixes a reduced experimental-validation subset and protocol (Table~\ref{tab:experimental_validation_subset}) that target the best-case hybrid claim, a representative median case, the least favourable direct-branch case, the grouped rough-treatment exceptions, the classical two-colour comparator case, and detector sensitivity against an independent reference chain on every core case. That first-pass reduced campaign is the natural next stage for broadening the claim: replace the present proxy-plus-rate-control hierarchy with an experimentally grounded fourth-order transfer function, propagate detector-side effects under laboratory-representative losses, accidentals, and phase-stepping imperfections, and test the hybrid branch on the reduced measured-surface subset. The public development repository is available at \url{https://github.com/dawidkucharski/Quantum_interference}, and the Q1 submission release is fixed by GitHub tag \texttt{submission-2026-04-28-q1} and archived on Zenodo with version DOI \url{https://doi.org/10.5281/zenodo.19852223} under concept DOI \url{https://doi.org/10.5281/zenodo.19844594}. The accompanying reproducibility asset provides the derived benchmark-grid surfaces and checksumed public CSV/table record. Together with the defined experimental subset, the present benchmark is a well-defined launch point for experimental hybrid coincidence-assisted profilometry.
...
\dataavailability{The Git-tracked project snapshot associated with this study contains the manuscript sources, simulation code, and the generated benchmark artefacts cited in the paper. Canonical regeneration is driven by \path{scripts/run_paper_measured_benchmark.py}, \path{scripts/run_paper_optprops.py}, and \path{scripts/select_experimental_validation_subset.py}; targeted controls, canonical sources, output folders, and revision-support files are enumerated in \path{docs/submission_snapshot_manifest.md}. The reduced experimental-validation subset is included under \path{outputs/paper_alicona_benchmark/experimental_validation/} together with the accompanying protocol in \path{docs/experimental_validation_protocol.md}. A public development repository is available at \url{https://github.com/dawidkucharski/Quantum_interference}, and the Q1 submission snapshot for the present manuscript is fixed by GitHub release \texttt{submission-2026-04-28-q1}. The release is archived on Zenodo with version DOI \url{https://doi.org/10.5281/zenodo.19852223} and concept DOI \url{https://doi.org/10.5281/zenodo.19844594}. A release companion archive, \path{quantum_interference_reproducibility_submission-2026-04-28-q1.tar.gz}, provides the derived $256\times256$ benchmark-grid surfaces, public CSV summaries, generated table sources, exact regeneration commands, and SHA-256 checksum \path{cbe60dc49cbe5d8b6c7253a05e3118fde4e86420e9d180a28c37979ba37bc83f}. The full local set of measured input \texttt{.sur} topographies is excluded from the normal Git history because the raw dataset is too large for a practical public GitHub snapshot; those raw SUR files remain available from the corresponding author upon reasonable request. The available public snapshot and companion archive support regeneration and audit of the simulation benchmark, figures, tables, derived benchmark-grid inputs, and reduced-validation planning record. Any future experimental validation dataset should be archived separately with the corresponding independent reference measurements.}
