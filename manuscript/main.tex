...
\abstract{This paper introduces a reproducible architecture-screening benchmark for rough-surface interferometric profilometry. Its contribution is an endpoint-level, matched-count comparison of complete reconstruction pipelines: classical four-step phase-shifting interferometry (PSI), direct coincidence-proxy reconstruction, and hybrid coarse-to-fine reconstruction applied to the same measured engineering textures. The comparison asks whether a coincidence-derived long-effective-wavelength observable should carry the final height map or serve as an ambiguity-resolving prior for short-wavelength PSI. Fifty-nine focus-variation (FV) topographies exported as Mountains/DigitalSurf \texttt{.sur} files provide common measured geometric priors for simulated optical observations. The main result is role separation. On the measured-surface benchmark, the hybrid branch gives the lowest median detrended height RMSE (314.0~nm), wins on 32 of 59 surfaces, remains below a classical-frontier oracle (559.1~nm), and keeps the same ordering in a rate-based coincidence control (290.9~nm ideal; 376.3~nm non-ideal). Roughness endpoints set the boundary: hybrid gives the lowest matched-bandwidth Sa and Sq errors (86.3 and 152.4~nm), whereas direct coincidence-proxy reconstruction is selectively best for Sz (2653.1~nm) but remains process dependent. Classical two-colour and classical-frontier controls show that long-wavelength envelope following alone is not a quantum-advantage signature. The benchmark supports a bounded architecture map: coincidence-derived information is most useful as a coarse prior inside a hybrid estimator, while final fine texture remains anchored by short-wavelength PSI. Because FV surfaces are geometric priors, not traceable standards, the output is a reproducible experimental-priority map for hardware follow-up.}
...
Phase-shifting interferometry (PSI) remains one of the standard high-precision optical routes for surface measurement, but practical deployment is limited by environmental sensitivity and non-ideal stepping in real optical testing \cite{McDonnell2020}. Several strategies have therefore been developed to stabilise or regularise PSI, including spatiotemporal phase-shifting methods \cite{Ri2020}, full-field least-squares phase retrieval \cite{Medina2014}, extended multi-frame algorithms that trade speed for robustness \cite{DeGroot1997283}, and calibration-oriented treatments for highly reflective surfaces \cite{Creath1999182}.
...
\conflictsofinterest{The author is listed as a Guest Editor of the \textit{Photonics} Special Issue ``Optical and Photonic Metrology: Science and Technology''. The author requests that editorial assignment, peer review, and publication decisions for this manuscript be handled independently by the \textit{Photonics} Editorial Office and editors not involved with the submission. The author declares no other conflicts of interest.}
