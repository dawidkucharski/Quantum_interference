...
def _write_latex_table(path: Path, rows: list[dict[str, object]]) -> None:
    lines = [
        ...
        "\\toprule",
        "Tier & Case & Selected surface & Purpose in reduced validation " + r"\\",
        "\\midrule",
    ]
    for row in rows:
        ...
        stem = str(row["stem"])
        material = _escape_latex(str(row["material"]))
        treatment = _escape_latex(str(row["treatment"]))
        surface_cell = f"\\path{{{stem}}}\\newline{{\\scriptsize {material}; {treatment}}}"
        purpose = _escape_latex(str(row["selection_rationale"]))
        lines.append(f"{tier} & {case} & {surface_cell} & {purpose} " + r"\\")
    lines.extend(
        [
            "\\bottomrule",
            "\\end{tabularx}",
